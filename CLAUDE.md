...
The simulation logs through the `log` crate; enable output with e.g. `RUST_LOG=info cargo run --example run_example_simulation`.
...
- `sessions: Vec<Session>` — append-only output log; each charging start or queue renege pushes one row.

`Simulation::run` pops events until the queue is empty. Three event types:
...
- `Renege` → remove the vehicle from the `waiting_queue` if present.
...
- Idle duration via gamma distribution (`distributions/idle_distr.rs`).
